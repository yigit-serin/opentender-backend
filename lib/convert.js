...
class Converter {
	...
	checkNUTS(doc) {
		...
		nutscodes = this.cleanNUTS(nutscodes);
		if (nutscodes && nutscodes.length > 0) {
			doc.nutscode = nutscodes[0];
		}
	}
	...
	cleanItem(doc) {
		...
		doc._indicators = this.getIndicatorsAsFields(doc.indicators);
		doc.scores = this.calculateIndicatorScores(doc.indicators);
		doc.documentsLocation = this.cleanAddress(doc.documentsLocation);
		doc.date = this.chooseTenderDate(doc);
		doc.mainCPV = this.chooseMainCVP(doc);
		this.shortenProperties(['buyerAssignedId'], doc, 200);
