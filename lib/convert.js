...
let getCountryCode = item => {
	if (item.modifiedBy !== 'eu.digiwhist.worker.eu.master.TedTenderMaster') {
		console.log('TODO1:', item.modifiedBy);
	} else {
		if (item && item.buyers && item.buyers.length > 0) {
...
let cleanRecursive = (o, level) => {
	...
	if (o['buyerAssignedId']) {
		if (o['buyerAssignedId'].length > 200) o['buyerAssignedId'] = o['buyerAssignedId'].slice(0, 200);
	}
	if (o['bidId']) {
		o['id'] = o['bidId'];
		delete o['bidId'];
