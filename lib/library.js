...
class Library {
	...
	constructor(config) {
		...
		this.cpv_list = JSON.parse(fs.readFileSync(path.resolve(config.data.path, 'cpv/cpvs_2008.json')).toString());
		this.nuts_names = JSON.parse(fs.readFileSync(path.resolve(config.data.shared, 'nuts/nuts_names.json')).toString());
		this.schema = JSON.parse(fs.readFileSync(path.resolve(config.data.shared, 'schema.json')).toString());
	...
	isKnownCPV(id) {
		if (id) {
			let s_id = id.slice(0, 8);
			let cpv = this.cpvs_divisions[s_id] || this.cpvs_groups[s_id] || this.cpvs_categories[s_id] || this.cpvs_full[s_id];
			return !!cpv;
		}
		return false;
	}
