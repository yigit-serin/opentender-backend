...
class Store {
	...
	constructor(config) {
		...
		this.Tender = new IndexStore(mapping.TENDER(config.country.code), client);
		this.Buyer = new IndexStore(mapping.BUYER(config.country.code), client);
		this.Supplier = new IndexStore(mapping.SUPPLIER(config.country.code), client);
		this.client = client;
	}
	...
	init(cb) {
		...
		this.client.ping({
			...
			(error) => {
				...
				async.forEachSeries(
					[this.Tender, this.Buyer, this.Supplier], (index, next) => {
