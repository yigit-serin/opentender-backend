...
const Aggregations = {
	top_sum_finalPrice_authorities: {
			...
			'aggregations': {
				'authorities_nested': {
					'terms': {
						...
						'shard_size': 0,
						'size': 10,
						'order': [{'sum_agg>sums_finalPrice': 'desc'}]
					},
					'aggregations': {
						'sum_agg': {
							'reverse_nested': {},
		...
		parse: (answer) => {
			...
			buckets.forEach(bucket => {
				if (bucket.sum_agg.sums_finalPrice.value > 0) {
					result.top10.push({
						value: bucket.sum_agg.sums_finalPrice.value,
						groupId: bucket.key
					});
				}
			});
	...
	sums_finalPrice: {
		...
		parse: (answer) => {
			...
			buckets.forEach(bucket => {
				result[bucket.key] = Utils.roundCurrencyValueFloat(bucket.sum_price.value);
			});
			return result;
		}
	},
	...
	histogram_lots_awardDecisionDate_finalPrices: {
		...
		parse: (answer) => {
			let result = {};
			let root = getAnswerAggreggation(answer, 'histogram_lots_awardDecisionDate_finalPrices');
			let buckets = root.dates_nested.buckets;
			buckets.forEach(bucket => {
				let year = parseInt(bucket.key_as_string.slice(0, 4), 10);
				if (Utils.isValidDigiwhistYear(year)) {
					let o = {value: bucket.doc_count, sums_finalPrice: {}, avgs_finalPrice: {}};
					let sub_buckets = bucket.tender_denested.sums_finalPrice.buckets;
					sub_buckets.forEach(sub_bucket => {
						o.sums_finalPrice[sub_bucket.key] = sub_bucket.sum_price.value;
						o.avgs_finalPrice[sub_bucket.key] = sub_bucket.avg_price.value;
					});
					result[year] = o;
				}
			});
