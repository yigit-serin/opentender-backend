...
const Aggregations = {
	...
	company_nuts: {
		...
		parse: (answer, cpvs) => {
			let result = {};
			answer.company_nuts.company_nuts_nested.buckets.sort((a, b) => {
				if (a.key < b.key) return -1;
				if (a.key > b.key) return 1;
				return 0;
			});
			answer.company_nuts.company_nuts_nested.buckets.forEach(bucket => {
	...
	authority_nuts: {
		...
		parse: (answer, cpvs) => {
			let result = {};
			answer.authority_nuts.authority_nuts_nested.buckets.sort((a, b) => {
				if (a.key < b.key) return -1;
				if (a.key > b.key) return 1;
				return 0;
			});
			answer.authority_nuts.authority_nuts_nested.buckets.forEach(bucket => {
				let nut = bucket.key.split('-')[0].trim();
				result[nut] = bucket.doc_count;
			});
			return result;
	...
	histogram_lots_awardDecisionDate: {
		...
		parse: (answer, cpvs) => {
			let result = {};
			let buckets = answer.histogram_lots_awardDecisionDate.dates_filter ?
				answer.histogram_lots_awardDecisionDate.dates_filter.dates_nested.buckets :
	...
	count_lots_bids: {
		...
		parse: (answer, cpvs, tendercount) => {
			let lotsbids = answer.count_lots_bids.lots_filter ? answer.count_lots_bids.lots_filter.lotsbids : answer.count_lots_bids.lotsbids;
			let result = {
