...
const Aggregations = {
	...
	terms_main_cpv_divisions_scores: {
		...
		parse: (answer, options) => {
			...
			buckets.forEach(bucket => {
				let scores = {};
				let subbuckets = bucket.tender_denested.avg_scores.avg_scores_nested.buckets;
				subbuckets.forEach(subbucket => {
					if (subbucket.avg_scores_nested_avg.value !== null) {
						scores[subbucket.key] = subbucket.avg_scores_nested_avg.value;
					}
				});
				result[bucket.key] = {
					name: options.library.getCPVName(bucket.key, options.lang),
	...
	terms_main_cpv_divisions_indicators: {
		...
		parse: (answer, options) => {
			...
			buckets.forEach(bucket => {
				let scores = {};
				let subbuckets = bucket.tender_denested.avg_indicators_scores.avg_indicators_scores_nested.buckets;
				subbuckets.forEach(subbucket => {
					if (subbucket.avg_indicators_scores_nested_avg.value !== null) {
						scores[subbucket.key] = subbucket.avg_indicators_scores_nested_avg.value;
					}
				});
				result[bucket.key] = {
					name: options.library.getCPVName(bucket.key, options.lang),
	...
	terms_company_nuts: {
		request: {
			...
			'aggregations': {
				'company_nuts_nested': {
					'terms': {
						'field': 'lots.bids.bidders.address.nutscode',
						'size': 3000000
					}
				}
			}
			...
			buckets.forEach(bucket => {
				...
				subbuckets.forEach(subbucket => {
					if (subbucket.avg_indicators_scores_nested_avg.value !== null) {
						...
						result[subbucket.key][year] = subbucket.avg_indicators_scores_nested_avg.value;
					}
				});
				...
				subbuckets.forEach(subbucket => {
					if (subbucket.avg_scores_nested_avg.value !== null) {
						result[subbucket.key] = result[subbucket.key] || {};
						result[subbucket.key][year] = subbucket.avg_scores_nested_avg.value;
					}
				});
			});
			return result;
...
const ParameterAggregations = {
	avg_score_para: {
					...
					'avg_score_nested': {
						'filter': {
							'query': {
								'bool': {
									'filter': [
										{'term': {'scores.type': params.name}}
									]
								}
							}
						},
						'aggregations': {
							'avg_score_nested_value': {
								'avg': {
									'field': 'scores.value'
								}
							}
		...
		parse: (answer, options, params) => {
			...
			let result = {};
			if (root.avg_score_nested.avg_score_nested_value.value !== null) {
				result[params.name] = root.avg_score_nested.avg_score_nested_value.value;
			}
			return result;
		}
	},
	avg_score_custom_para: {
		request: (params) => {
			let fields = Object.keys(params.custom).map(key => '_indicators.' + key);
	...
	avg_indicator_para: {
		...
		parse: (answer, options, params) => {
			...
			let result = {};
			if (root.avg_indicator_nested.avg_indicator_nested_value.value !== null) {
				result[params.name] = root.avg_indicator_nested.avg_indicator_nested_value.value;
			}
			return result;
		}
	},
	...
	histogram_indicators_para: {
		...
		parse: (answer, options, params) => {
			...
			buckets.forEach(bucket => {
				...
				if (bucket.avg_scores.avg_scores_nested.avg_scores_nested_value.value !== null) {
					result[params.name] = result[params.name] || {};
					result[params.name][year] = bucket.avg_scores.avg_scores_nested.avg_scores_nested_value.value;
				}
			});
			return result;
		}
		...
		parse: (answer, options, params) => {
			...
			buckets.forEach(bucket => {
				...
				if (bucket.avg_indicator_scores.avg_indicator_scores_nested.avg_indicator_scores_nested_value.value !== null) {
					result[params.name] = result[params.name] || {};
					result[params.name][year] = bucket.avg_indicator_scores.avg_indicator_scores_nested.avg_indicator_scores_nested_value.value;
				}
			});
			return result;
		}
	...
	terms_main_cpv_divisions_score_para: {
		...
		parse: (answer, options, params) => {
			...
			let buckets = root.cpvs_filter.cpvs.buckets;
			buckets.forEach(bucket => {
				if (bucket.tender_denested.avg_score.avg_score_nested.avg_score_nested_value.value !== null) {
					result[bucket.key] = {
						name: options.library.getCPVName(bucket.key, options.lang),
						value: bucket.tender_denested.avg_score.avg_score_nested.avg_score_nested_value.value
					};
				}
			});
			return result;
	...
	terms_main_cpv_divisions_score_custom_para: {
		...
		parse: (answer, options, params) => {
			...
			let buckets = root.cpvs_filter.cpvs.buckets;
			buckets.forEach(bucket => {
				let score = ParameterAggregations.avg_score_custom_para.parse(bucket.tender_denested, options, params);
				if (score[params.name] !== null) {
					result[bucket.key] = {
						name: options.library.getCPVName(bucket.key, options.lang),
						value: score[params.name]
					};
				}
			});
			return result;
	...
	terms_main_cpv_divisions_indicator_para: {
		...
		parse: (answer, options, params) => {
			...
			let buckets = root.cpvs_filter.cpvs.buckets;
			buckets.forEach(bucket => {
				if (bucket.tender_denested.avg_indicator.avg_indicator_nested.avg_indicator_nested_value.value !== null) {
					result[bucket.key] = {
						name: options.library.getCPVName(bucket.key, options.lang),
						value: bucket.tender_denested.avg_indicator.avg_indicator_nested.avg_indicator_nested_value.value
					};
				}
			});
			return result;
...
const Filters = {
	...
	byAuthorityNuts: (nutscode, level) => {
		let term = {};
		let field = 'buyers.address.nutscode';
...
let buildSearchFilter = (filter) => {
	...
	let buildSearchFilterInternal = f => {
		if (f.type === 'select') {
			...
			if (f.weights) {
				let fields = Object.keys(f.weights).map(key => '_indicators.' + key);
