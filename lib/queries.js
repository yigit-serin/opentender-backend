	...
	avg_score_para: {
		...
		parse: (answer, options, params) => {
			...
			if (root.avg_score_nested.avg_score_nested_value.value !== null) {
				result[params.name] = Utils.roundValueTwoDecimals(root.avg_score_nested.avg_score_nested_value.value);
			}
			return result;
		}
	},
	...
	avg_indicator_para: {
		...
		parse: (answer, options, params) => {
			...
			if (root.avg_indicator_nested.avg_indicator_nested_value.value !== null) {
				result[params.name] = Utils.roundValueTwoDecimals(root.avg_indicator_nested.avg_indicator_nested_value.value);
			}
			return result;
		}
	},
		...
		parse: (answer, options, params) => {
			...
			buckets.forEach(bucket => {
				...
				if (bucket.avg_scores.avg_scores_nested.avg_scores_nested_value.value !== null) {
					...
					result[params.name][year] = Utils.roundValueTwoDecimals(bucket.avg_scores.avg_scores_nested.avg_scores_nested_value.value);
				}
			});
			return result;
		}
		...
		parse: (answer, options, params) => {
			...
			buckets.forEach(bucket => {
				if (bucket.avg_score.avg_score_nested.avg_score_nested_value.value !== null) {
					result[bucket.key] = {
						name: options.library.getCPVName(bucket.key, options.lang),
						value: Utils.roundValueTwoDecimals(bucket.avg_score.avg_score_nested.avg_score_nested_value.value)
					};
				}
			});
			return result;
