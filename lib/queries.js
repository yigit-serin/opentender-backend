...
const Aggregations = {
    top_sum_finalPrice_authorities: {
        ...
        parse: (answer, options) => {
            ...
            let buckets = root.buckets ? root.buckets[0].authorities_nested.buckets: root.authorities_nested.buckets;
    ...
    top_sum_finalPrice_companies: {
        ...
        parse: (answer, options) => {
            ...
            let buckets = root.buckets ? root.buckets[0].companies_nested.buckets : root.companies_nested.buckets;
            buckets.forEach(bucket => {
                if (bucket.sum_agg.sum_finalPriceEUR.value > 0) {
                    result.top10.push({
                        value: Utils.roundValueTwoDecimals(bucket.sum_agg.sum_finalPriceEUR.value),
        ...
        request: {
            ...
            'aggregations': {
                'terms_indicators_nested': {
                    'terms': {
                        'field': 'ot.indicators.type.keyword',
                        'size': 100,
                    },
                },
            },
    ...
    terms_authority_nuts: {
        ...
        parse: (answer, options) => {
	        ...
	        let buckets = root.buckets
                ? root.buckets[0].authority_nuts1_nested.buckets.concat(root.buckets[0].authority_nuts2_nested.buckets).concat(root.buckets[0].authority_nuts3_nested.buckets).concat(root.buckets[0].authority_nuts0_nested.buckets)
                : root.authority_nuts1_nested.buckets.concat(root.authority_nuts2_nested.buckets).concat(root.authority_nuts3_nested.buckets).concat(root.authority_nuts0_nested.buckets)
            buckets.sort((a, b) => {
                if (a.key < b.key) {
                    return -1;
                }
    ...
    histogram: {
        request: {
            'date_histogram': {'field': 'ot.date', 'calendar_interval': 'year'}
        },
        parse: (answer, options) => {
            let result = {};
...
const ParameterAggregations = {
    ...
    avg_score_custom_para: {
        request: (params) => {
            const parseParamsForScriptedMetricCustom = () => {
                let resultString = '[';
                let isEnd = false;
                const weights = Object.entries(params.custom);
                for (const [index, value] of Object.values(params.custom).entries()) {
                    isEnd = index >= weights.length - 1;
                    const keyIndicator = weights[index][0];

                    if (isEnd) {
                        resultString += `\'${keyIndicator}\': ${value}]`;
                    } else {
                        resultString += `\'${keyIndicator}\': ${value}, `;
                    }
                }
            ...
            return {
                scripted_metric: {
                    init_script: "state.indicators = [];",
                    map_script: "state.indicators.add(params._source.ot.indicator)",
                    combine_script: `
						double sum = 0; 
						double count = 0; 
						...
						double final_sum = 0; 
						def weights = ${parseParamsForScriptedMetricCustom()}; 
						
						for (indicators in state.indicators) {
							for (key in weights.keySet()) { 
								if (indicators.get(key) == null) {
									continue;
								}
								 
								weight_value = (weights[key] != null ? weights[key] : 1); 
								indicator_value = indicators[key];
								 
								if (!indicator_value.isNaN()) {
									sum += (indicator_value * weight_value); 
									count += 1; 
								}
							}
						...
						return ['sum': final_sum, 'count': state.indicators.size()];`,
                    reduce_script: `
						double sum = 0; 
						double count = 0;
                    ...
                    'avg_indicator_scores': {
                        ...
                        'aggregations': {
                            'avg_indicator_scores_nested': {
                                'filter': {
                                    'term': { 'indicators.type.keyword': params.name }
                                },
                                'aggregations': {
...
let buildSearchFilter = (filter) => {
        ...
        if (values.length > 0) {
            ...
            values.forEach(v => {
                ...
                if (value !== null) {
                    ...
                    b.bool.should.push(m);
                }
            });
            if (b.bool.should.length === 1) {
                return b.bool.should[0];
            }
            if (b.bool.should.length === 0) {
                return null;
            }
            return b;
        }
    ...
    let buildSearchFilterInternal = f => {
        if (f.type === 'select') {
            ...
            if (f.weights) {
                const parseParamsForScriptCustom = () => {
                    let resultString = '[';
                    let isEnd = false;
                    const weights = Object.entries(f.weights);
                    for (const [index, value] of Object.values(f.weights).entries()) {
                        isEnd = index >= weights.length - 1;
                        const keyIndicator = 'ot.indicator.' + weights[index][0];
                ...
                let m = {
                    script: {
                        script: `
							double gte = ${f.value[0]}; 
							double lte = ${f.value[1]}; 
							double sum = 0; 
							double count = 0; 
							double indicator_value = 0; 
							double weight_value = 1; 
							double final_sum = 0; 
							def weights = ${parseParamsForScriptCustom()}; 
							for (key in weights.keySet()) { 
								if (doc[key].size() == 0) {
									continue;
								}
...
let buildSearchAggregations = (options) => {
    ...
    let resolveAgg = (agg, node) => {
        ...
        if (nested) {
            ...
            if (agg.type === 'years') {
                aa.aggregations[name + '_nested'] = {
                    'date_histogram': {'field': agg.field, 'calendar_interval': 'year'}
                };
        ...
        if (agg.type === 'sum') {
            ...
            node[name + '_over_time'] = {'date_histogram': {'field': agg.field, 'calendar_interval': 'year'}};
            return;
        } else if (agg.type === 'range') {
