...
const ParameterAggregations = {
					...
					'avg_score_nested': {
						'filter': {
							'query': {
								'bool': {
									'filter': [
										{'term': {'ot.scores.type': params.name}}
									]
								}
							}
						},
	...
	avg_score_custom_para: {
		request: (params) => {
			let fields = Object.keys(params.custom).map(key => 'ot.indicators.' + key);
	...
	terms_main_cpv_divisions_score_para: {
		...
		parse: (answer, options, params) => {
			...
			let buckets = root.buckets;
			buckets.forEach(bucket => {
				if (bucket.avg_score.avg_score_nested.avg_score_nested_value.value !== null) {
					result[bucket.key] = {
						name: options.library.getCPVName(bucket.key, options.lang),
						value: bucket.avg_score.avg_score_nested.avg_score_nested_value.value
					};
				}
			});
			return result;
		...
		parse: (answer, options, params) => {
			...
			buckets.forEach(bucket => {
				if (bucket.avg_indicator.avg_indicator_nested.avg_indicator_nested_value.value !== null) {
					result[bucket.key] = {
						name: options.library.getCPVName(bucket.key, options.lang),
						value: bucket.avg_indicator.avg_indicator_nested.avg_indicator_nested_value.value
					};
				}
			});
			return result;
...
let buildSearchFilter = (filter) => {
	...
	let buildSearchFilterInternal = f => {
		if (f.type === 'select') {
			...
			if (f.weights) {
				let fields = Object.keys(f.weights).map(key => 'ot.indicators.' + key);
