...
const Aggregations = {
	...
	histogram_lots_awardDecisionDate_scores: {
		...
		parse: (answer) => {
			let result = {};
			let root = getAnswerAggreggation(answer, 'histogram_lots_awardDecisionDate_scores');
			let buckets = root.dates_nested.buckets;
			buckets.forEach(bucket => {
				let year = parseInt(bucket.key_as_string.slice(0, 4), 10);
				if (Utils.isValidDigiwhistYear(year)) {
	...
	histogram_lots_awardDecisionDate_indicators: {
		request: {
			'nested': {
				'path': 'lots'
			},
		...
		parse: (answer) => {
			let result = {};
			let root = getAnswerAggreggation(answer, 'histogram_lots_awardDecisionDate_indicators');
			let buckets = root.dates_nested.buckets;
			buckets.forEach(bucket => {
				let year = parseInt(bucket.key_as_string.slice(0, 4), 10);
				if (Utils.isValidDigiwhistYear(year)) {
