...
const ParameterAggregations = {
	avg_score_para: {
		...
		parse: (answer) => {
			...
			let result = {};
			Object.keys(root.avg_score_nested).forEach(key => {
				let o = root.avg_score_nested[key];
				if (o && typeof o === 'object' && o.hasOwnProperty('value') && o.value !== null) {
					result[key] = o.value;
				}
			});
			return result;
	...
	avg_indicator_para: {
					...
					'avg_indicator_nested': {
						'filter': {
							'query': {
								'bool': {
									'filter': [
										{'term': {'indicators.type': fields['name']}}
									]
								}
							}
						},
		...
		parse: (answer) => {
			...
			let result = {};
			Object.keys(root.avg_indicator_nested).forEach(key => {
				let o = root.avg_indicator_nested[key];
				if (o && typeof o === 'object' && o.hasOwnProperty('value') && o.value !== null) {
					result[key] = o.value;
				}
			});
					...
					'terms_indicators_score_nested': {
						'filter': {
							'query': {
								'bool': {
									'filter': [
										{'match_phrase_prefix': {'indicators.type': fields['name']}}
									]
								}
							}
						},
	...
	histogram_indicators_para: {
		...
		parse: (answer) => {
			...
			buckets.forEach(bucket => {
				...
				Object.keys(bucket.avg_scores.avg_scores_nested).forEach(key => {
					let o = bucket.avg_scores.avg_scores_nested[key];
					if (o && typeof o === 'object' && o.hasOwnProperty('value') && o.value !== null) {
						result[key] = result[key] || {};
						result[key][year] = o.value;
					}
				});
			});
			return result;
	...
	histogram_indicator_para: {
		...
		parse: (answer) => {
			...
			buckets.forEach(bucket => {
				let year = parseInt(bucket.key_as_string.slice(0, 4), 10);
				Object.keys(bucket.avg_indicator_scores.avg_indicator_scores_nested).forEach(key => {
					let o = bucket.avg_indicator_scores.avg_indicator_scores_nested[key];
					if (o && typeof o === 'object' && o.hasOwnProperty('value') && o.value !== null) {
						result[key] = result[key] || {};
						result[key][year] = o.value;
					}
	...
	terms_main_cpv_divisions_score_para: {
		...
		parse: (answer, options) => {
			...
			buckets.forEach(bucket => {
				let value = null;
				Object.keys(bucket.tender_denested.avg_score.avg_score_nested).forEach(key => {
					let o = bucket.tender_denested.avg_score.avg_score_nested[key];
					if (o && typeof o === 'object' && o.hasOwnProperty('value') && o.value !== null) {
						value = o.value;
					}
				});
	...
	terms_main_cpv_divisions_indicator_para: {
		...
		parse: (answer, options) => {
			...
			buckets.forEach(bucket => {
				let value = null;
				Object.keys(bucket.tender_denested.avg_indicator.avg_indicator_nested).forEach(key => {
					let o = bucket.tender_denested.avg_indicator.avg_indicator_nested[key];
					if (o && typeof o === 'object' && o.hasOwnProperty('value') && o.value !== null) {
						value = o.value;
					}
				});
