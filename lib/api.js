...
class Api {
	...
	getMarketAnalysisStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, data) => {
			...
			let stats = aggs.parse(data.aggregations, this.library);
	...
	getSectorStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, data) => {
			...
			let stats = aggs.parse(data.aggregations, this.library, data.hits.total);
			...
			this.store.Tender.search(b, 0, 0, (err, full_result) => {
				...
				let full_data = aggs.parse(full_result.aggregations, this.library, full_result.hits.total);
			...
			result.hits.hits = result.hits.hits.map(doc => {
				if (doc._source.cpvs) {
					doc._source.cpvs.forEach(cpv => {
						cpv['name'] = this.library.getCPVName(cpv.code, 'EN');
					});
				}
				return doc._source;
			});
	...
	getCPV(id, cb) {
		...
		let cpvInfo = this.library.parseCPVs(id, 'EN');
		if (!cpvInfo.cpv) {
			return cb(404);
		}
