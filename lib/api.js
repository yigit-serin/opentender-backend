...
const Api = function (config) {
	...
	this.getIndicatorStats = (options, country_id, cb) => {
		...
		b.aggregations = getAggregations(corruptionAggregations);

		if (b.query.filtered.filter.bool) {
			//apply nested filter to aggregations, too
			let q = b.query.filtered.filter.bool.must[1].nested;
			Object.keys(b.aggregations).forEach(key => {
				let agg = b.aggregations[key];
