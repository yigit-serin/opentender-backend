...
class Api {
	...
	getIndicatorStats(options, country_id, cb) {
		...
		if (b.query.bool && b.query.bool.filter) {
			b.query.bool.filter.forEach(filter => {
				...
				list.forEach(f => {
					...
					b.aggregations['terms_indicators']['terms_indicators_nested'].terms.include = indicator_type + '.*';
				});
			});
		}
		// console.log('a', JSON.stringify(b.query, null, '\t'));
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			aggs = Queries.buildAggregations(['histogram_lots_awardDecisionDate', 'terms_main_cpv_divisions', 'terms_indicators_score', 'avg_scores']);
			b.aggregations = aggs.request;
			...
			this.store.Tender.search(b, 0, 0, (err, full_result) => {
				...
				stats.terms_indicators_score = data.terms_indicators_score;
