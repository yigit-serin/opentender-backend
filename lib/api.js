...
const Api = function (config) {
	...
	this.getCPVStats = (options, country_id, cb) => {
		...
		let cpvInfo = library.parseCPVs(id);
		if (!cpvInfo.cpv) {
			return cb(404);
		}
		let b = {};
		...
		store.Tender.search(b, 0, 0, (err, result) => {
			...
			if (cpvInfo.parent_cpv) {
				parent_cpv = {id: id.slice(0, 2), name: library.getCPVName(cpvInfo.parent_cpv.id, lang)};
			}
			cb(null, {sector: {id: id, name: library.getCPVName(id, lang)}, parent: parent_cpv, stats: stats});
		});
