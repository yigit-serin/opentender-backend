...
const Api = function (config) {
	...
	this.getCPVStats = (options, country_id, cb) => {
		...
		let b = {};
		b.query = id.length === 2 ? Queries.Filters.byMainCPVdivision(id) : Queries.Filters.byMainCPVfull(id);
	...
	this.getCPVUsageStats = (country_id, cb) => {
		this.getAggregation(country_id, 'terms_main_cpvs', cb);
	};
	...
	this.getMarketAnalysisStats = (options, country_id, cb) => {
		let aggs = Queries.buildAggregations(['terms_main_cpvs']);
		let stats_aggs = Queries.buildAggregations(['top_authorities', 'top_companies', 'sums_finalPrice', 'terms_main_cpvs', 'histogram_lots_awardDecisionDate', 'count_lots_bids']);
		aggs.request['terms_main_cpvs'].aggregations.cpvs_filter.aggregations.maincpvs.aggregations = {
			'tender_denested': {
				'reverse_nested': {},
