...
class Api {
	...
	getIndicatorStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			this.store.Tender.search(b, 0, 0, (err, full_result) => {
				...
				delete stats.histogram_lots_awardDecisionDate_finalPrices;
	...
	getMarketAnalysisStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, data) => {
			...
			delete stats.terms_main_cpv_divisions;
	...
	getSectorStats(options, country_id, cb) {
		...
		let aggIds = ['top_authorities', 'top_companies', 'sums_finalPrice', 'histogram_lots_awardDecisionDate_finalPrices', 'count_lots_bids'];
	...
	getCompanyStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			stats.top_authorities = Queries.Aggregations.top_authorities.parse(answer_winning.tender_denested, parseoptions);
