...
class Api {
	...
	searchTender(options, country_id, cb) {
		...
		this.store.Tender.search(body, (options.size || 10), (options.from || 0), (err, result) => {
			...
			result.hits.hits = result.hits.hits.map(doc => {
				if (doc._source.cpvs) {
					doc._source.cpvs.forEach(cpv => {
						cpv['name'] = this.library.getCPVName(cpv.code, options.lang);
					});
				}
				return doc._source;
			});
	...
	fillAggregationAuthorityBodies(stats, cb) {
		...
		this.store.Authority.search(body, 0, 0, (err, result) => {
				...
				let results = {};
				result.aggregations.first_entry.buckets.forEach(bucket => {
					results[bucket.key] = bucket.hits.hits.hits[0]._source.body;
				});
