...
class Api {
            ...
            if (err) {
                ...
                if (result.hits.total < 1) {
                    ...
                    tender.suppliers = suppliers.map((supplier) => ({
                        id: supplier.id,
                        name: supplier.name,
                        address: (supplier.address && supplier.address.rawAddress),
    ...
    saveFormDownloads(req, cb) {
        ...
        if (dataRequest.id) {
            this.store.Downloads.get(dataRequest.id, (error, result) => {
                if (!result.hits.total) {
                    return cb(404);
                }

                let doc = result.hits.hits[0];
                let data = result.hits.hits[0]._source;

                data.download_attempts ++;
                data.updated_at = nowDateTime;

                this.store.Downloads.update(doc._id, data, (err, result) => {
                    if (err) {
                        return cb(err);
                    }
            ...
            if (!enumSector.includes(dataRequest.sector)) {
                return cb(400);
            }
            ...
            data.organization = dataRequest.organization;
            data.sector = dataRequest.sector;
            data.email = dataRequest.email;
            data.datasets_download = dataRequest.datasets_download;
            data.download_attempts = 1
            data.created_at = nowDateTime;
            data.updated_at = nowDateTime;

            this.store.Downloads.add(data, (err, result) => {
                if (err) {
                    return cb(err);
                }
    ...
    exportSaveFormDownloads(req, res, cd) {
        if (!req.query.exportKey || config.exportKey !== req.query.exportKey) {
            return res.status(400).json({ error: 'Check exportKey'})
        }

        let downloadsData = [];

        this.store.Downloads.streamQuery(2, {},(items, total) => {
            items.forEach(item => {
                downloadsData.push(item._source);
            })
        ...
        (error) => {
            ...
            downloadsData.forEach((item) => {
                item.datasets_download = item.datasets_download.join('; ');
            ...
            if (downloadsData.length) {
                const workSheet = XLSX.utils.json_to_sheet(downloadsData),
                    workBook = XLSX.utils.book_new(),
                    fileName = 'DownloadDatasets.xlsx',
                    filePath = `${fileName}`,
                    paramsAccess = {
                    ...
                    s3 = new AWS.S3(paramsAccess);

                XLSX.utils.book_append_sheet(workBook, workSheet, "Downloads datasets");
                ...
                res.attachment(params.Key).status(200);
                let fileStream = s3.getObject({Bucket: params.Bucket, Key: params.Key}).createReadStream();
                fileStream.pipe(res);
            } else {
