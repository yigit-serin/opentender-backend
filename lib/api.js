...
class Api {
	...
	getIndicatorStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			this.store.Tender.search(b, 0, 0, (err, full_result) => {
				...
				stats.histogram_indicators = data.histogram_indicators;
	...
	getCompanyStats(options, country_id, cb) {
		...
		let subaggs = Queries.buildAggregations(['terms_authority_nuts', 'top_terms_authorities', 'top_sum_finalPrice_authorities', 'histogram_finalPriceEUR']);
		b.aggregations = aggs.request;
	...
	getAuthorityStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			this.fillAggregationBodies(stats, (err) => {
				...
				let benchmark_body = {aggregations: benchmark_aggs.request};
				b.query = {'term': {'country': country_id}};
