...
class Api {
	...
	getIndicatorStats(options, country_id, cb) {
		...
		let indicatorFilter = (options.filters || []).find(f => f.field === 'scores.type' || f.field === 'indicators.type');
		if (indicatorFilter) {
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			if (b.query.bool && b.query.bool.filter) {
				b.query.bool.filter = b.query.bool.filter.filter(filter => {
					return (!(filter.nested && ['indicators', 'scores'].indexOf(filter.nested.path) >= 0)) && (!filter.script);
				});
