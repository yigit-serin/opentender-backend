...
class Api {
	...
	getMarketAnalysisStats(options, country_id, cb) {
		let aggs = Queries.buildAggregations(['terms_main_cpv_divisions', 'histogram_lots_awardDecisionDate']);
		...
		this.store.Tender.search(b, 0, 0, (err, data) => {
			...
			let result = data.aggregations['terms_main_cpv_divisions'].cpvs_filter.cpvs.buckets.map(bucket => {
				let sector = {
					id: bucket.key,
