...
const Api = function (config) {
	...
	const constAggregations = {
		...
		// 'dates': {
		// 	'date_histogram': {'field': 'bidDeadline', 'interval': 'year'}
		// },
		'lots': {
			'nested': {
				'path': 'lots'
	...
	let parseStatsAggregations = (agg_ids, aggregations, tender_count) => {
		...
		if (agg_ids.indexOf('dates') >= 0) {
			stats.lots_in_years = {};
			let buckets = aggregations.dates.dates_filter ? aggregations.dates.dates_filter.dates_nested.buckets : aggregations.dates.dates_nested.buckets;
			if (buckets) {
				buckets.forEach(bucket => {
					let year = parseInt(bucket.key_as_string.slice(0, 4), 10);
					if (isValidDigiwhistYear(year)) {
						stats.lots_in_years[year] = bucket.doc_count;
					}
				});
			}
		}
		if (agg_ids.indexOf('dates_by_cor_risk') >= 0) {
			stats.lots_with_cor_risk_in_years = {};
			let buckets = aggregations.dates_by_cor_risk.filtered_nested.dates_nested.buckets;
			if (buckets) {
				buckets.forEach(bucket => {
					let year = parseInt(bucket.key_as_string.slice(0, 4), 10);
					if (isValidDigiwhistYear(year)) {
						stats.lots_with_cor_risk_in_years[year] = bucket.doc_count;
					}
				});
			}
		}
		...
		if (agg_ids.indexOf('company_nuts') >= 0) {
			stats.company_nuts = {};
			aggregations.company_nuts.company_nuts_nested.buckets.sort((a, b) => {
				if (a.key < b.key) return -1;
				if (a.key > b.key) return 1;
				return 0;
			});
			aggregations.company_nuts.company_nuts_nested.buckets.forEach(bucket => {
				let nut = bucket.key.split('-')[0].trim();
				stats.company_nuts[nut] = bucket.doc_count;
			});
		}
		if (agg_ids.indexOf('authority_nuts') >= 0) {
			stats.authority_nuts = {};
			aggregations.authority_nuts.authority_nuts_nested.buckets.sort((a, b) => {
				if (a.key < b.key) return -1;
				if (a.key > b.key) return 1;
				return 0;
			});
			aggregations.authority_nuts.authority_nuts_nested.buckets.forEach(bucket => {
				let nut = bucket.key.split('-')[0].trim();
				stats.authority_nuts[nut] = bucket.doc_count;
			});
		}
		if (agg_ids.indexOf('indicators') >= 0) {
			stats.indicators = {};
			let buckets = aggregations.indicators.indicators_filter ? aggregations.indicators.indicators_filter.buckets : aggregations.indicators.buckets;
			buckets.forEach(bucket => {
				stats.indicators[bucket.key] = bucket.doc_count;
			});
		}
		if (agg_ids.indexOf('lots') >= 0) {
			let lotsbids = aggregations.lots.lots_filter ? aggregations.lots.lots_filter.lotsbids : aggregations.lots.lotsbids;
			stats.counts = {
	...
	let getAggregations = (agg_names) => {
		...
		agg_names.forEach(key => {
			result[key] = clone(constAggregations[key]);
		});
		return result;
	};
	...
	this.getViz = (ids, country_id, cb) => {
		...
		async.forEachSeries(viz, (v, next) => {
			...
			} else if (v === 'tender_lots_per_year') {
				me.getTenderPerYearStats(country_id, (err, data) => {
					if (err) return cb(err);
					result[v] = data;
					next();
				});
	...
	this.getCompanyStats = (ids, country_id, cb) => {
		...
		store.Tender.search(b, 0, 0, (err, result) => {
			...
			result.aggregations.lotsbids.lotsbids_nested_filter.lotsbids_nested.lotsbids_nested_filter.lots_denested.dates.buckets.forEach(bucket => {
				let year = parseInt(bucket.key_as_string.slice(0, 4), 10);
				if (isValidDigiwhistYear(year)) {
					stats.bids_in_years[year] = bucket.doc_count;
				}
			});
			stats.counts = {
	...
	this.getTenderPerYearStats = (country_id, cb) => {
		let b = {
			'aggregations': getAggregations(['dates'])
		};
		if (country_id) {
			b.query = {
				term: {
		...
		store.Tender.search(b, 0, 0, (err, result) => {
			...
			let stats = parseStatsAggregations(['dates'], result.aggregations, result.hits.total);
			cb(null, stats);
		});
	};
	...
	this.getIndicatorStats = (options, country_id, cb) => {
		const indicatorAggregations = ['authorities', 'companies', 'currencies', 'cpvs', 'dates', 'indicators', 'lots', 'dates'];
		let b = buildCountrySearchBody(options, country_id);
		b.aggregations = getAggregations(indicatorAggregations);
		if (b.query.filtered && b.query.filtered.filter.bool && b.query.filtered.filter.bool.must
			...
			applyNestedFilter(b, b.query.filtered.filter.bool.must[1].nested, '');
		}
		...
		if (or) {
			...
			b.aggregations.indicators.terms.include = indicator_type + '.*';
		}
		// console.log('query', JSON.stringify(b.query, null, '\t'));
		// console.log('aggregations', JSON.stringify(b.aggregations, null, '\t'));
		...
		store.Tender.search(b, 0, 0, (err, result) => {
			...
			let stats = parseStatsAggregations(indicatorAggregations, result.aggregations, result.hits.total);
			...
			// console.log('query', JSON.stringify(b.query, null, '\t'));
			b.aggregations = getAggregations(['dates', 'cpvs']);
			...
			store.Tender.search(b, 0, 0, (err, full_result) => {
				...
				let data = parseStatsAggregations(['dates', 'cpvs'], full_result.aggregations, full_result.hits.total);

				let histo_pc_per_year = {};
				Object.keys(data.lots_in_years).forEach(year => {
					let total = (data.lots_in_years[year] || 0);
					let value = (stats.lots_in_years[year] || 0);
					if (total > 0 && value > 0) {
				...
				stats.lots_pc_in_years = histo_pc_per_year;

				Object.keys(stats.cpvs).forEach(key => {
					let c = stats.cpvs[key];
					c.total = 0;
					c.percent = 0;
					if (data.cpvs[key]) {
						c.total = data.cpvs[key].value;
						if (c.total > 0) {
							c.percent = Math.round((c.value / (c.total / 100)) * 100) / 100;
						}
					}
				});
	...
	this.getAggregations = (country_id, aggregations, cb) => {
		let b = {
			query: {
				'match_all': {}
			},
		...
		store.Tender.search(b, 0, 0, (err, result) => {
			...
			let stats = parseStatsAggregations(aggregations, result.aggregations, result.hits.total);
			cb(null, stats);
		});
	};

	this.getPerCentCorruptionRiskIndicatorByYear = (country_id, cb) => {
		this.getAggregations(country_id, ['dates', 'dates_by_cor_risk'], function (err, stats) {
			if (err) {
				console.log(err);
				return cb(err);
			}
			let histo_pc_per_year = {};
			Object.keys(stats.lots_in_years).forEach(year => {
				let total = (stats.lots_in_years[year] || 0);
				let value = (stats.lots_with_cor_risk_in_years[year] || 0);
				if (total > 0) {
					histo_pc_per_year[year] = {percent: 0, value: 0, total: 0};
				} else {
					histo_pc_per_year[year] = {
						percent: Math.round((value / (total / 100)) * 100) / 100,
						value: value,
			...
			cb(null, {lots_pc_with_cor_risk_in_years: histo_pc_per_year});
		});

	};

	this.getCompanyNuts = (country_id, cb) => {
		this.getAggregations(country_id, ['company_nuts'], function (err, stats) {
			if (err) {
				console.log(err);
				return cb(err);
			}
			cb(null, stats.company_nuts);
		});

	};

	this.getAuthorityNuts = (country_id, cb) => {
		this.getAggregations(country_id, ['authority_nuts'], function (err, stats) {
			if (err) {
				console.log(err);
				return cb(err);
			}
			cb(null, stats.authority_nuts);
		});
	};
	...
	this.init = cb => {
		store.init(err => {
			// this.getIndicatorStats({}, 'DE', (err, data) => {
			// 	console.log(err, data);
			// });
			// this.getTopExpensiveTenders('DE', () => {
			// });
			// this.getAuthorityNuts('DE', (err, stats) => {
