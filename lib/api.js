...
class Api {
	...
	getIndicatorStats(options, country_id, cb) {
		...
		// console.log('a', JSON.stringify(b.aggregations, null, '\t'));
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			aggs = Queries.buildAggregations(['histogram_lots_awardDecisionDate', 'terms_main_cpv_divisions', 'terms_indicators_score']);
			b.aggregations = aggs.request;
			...
			this.store.Tender.search(b, 0, 0, (err, full_result) => {
				...
				Object.keys(data.terms_indicators_score).forEach(key => {
					console.log(key.indexOf(indicator), key, indicator);
