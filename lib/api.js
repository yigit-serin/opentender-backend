...
class Api {
	...
	getIndicatorStats(options, country_id, cb) {
		let aggs = Queries.buildAggregations([
			...
			'histogram_lots_awardDecisionDate_finalPrices']);
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			this.store.Tender.search(b, 0, 0, (err, full_result) => {
				...
				let stats_histogram = stats['histogram_lots_awardDecisionDate_finalPrices'];
				...
				Object.keys(data_histogram).forEach(year => {
					...
					if (total > 0 && value > 0) {
						histo_pc_per_year[year] = {
							...
							avgs_finalPrice: stats_histogram[year].avgs_finalPrice
						};
					}
				});
				stats.histogram_lots_awardDecisionDate_finalPrices = undefined;
	...
	getMarketAnalysisStats(options, country_id, cb) {
		...
		let cpv_stats_aggs = Queries.buildAggregations(['sums_finalPrice']);
		let nuts_stats_aggs = Queries.buildAggregations(['sums_finalPrice']);
	...
	getCompanyStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			stats.sums_finalPrice = Queries.Aggregations.sums_finalPrice.parse(answer_winning.tender_denested, parseoptions);
