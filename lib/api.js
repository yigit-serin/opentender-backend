...
class Api {
	...
	getIndicatorStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			this.store.Tender.search(b, 0, 0, (err, full_result) => {
				...
				// TODO: drydock fake indicator score calculation, change this to actual data use if the indicator format has changed
				stats.terms_indicators_score = {};
				Object.keys(stats.terms_indicators).forEach(key => {
					stats.terms_indicators_score[key] = stats.terms_indicators[key] / (full_result.hits.total / 100);
				});
				console.log(stats.terms_indicators_score);
				stats.terms_indicators = undefined;
	...
	getMarketAnalysisStats(options, country_id, cb) {
		let aggs = Queries.buildAggregations(['terms_main_cpv_divisions', 'histogram_lots_awardDecisionDate', 'terms_authority_nuts']);
	...
	getSectorStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, data) => {
			...
			// console.log('old query', JSON.stringify(b.query, null, '\t'));
	...
	getCompanyStats(options, country_id, cb) {
									...
									'aggregations': {
										...
										'tender_denested': {
											...
											'aggregations': {
												...
												'top_terms_authorities': Queries.Aggregations.top_terms_authorities.request,
												'top_sum_finalPrice_authorities': Queries.Aggregations.top_sum_finalPrice_authorities.request,
												'sum_finalPriceEUR': Queries.Aggregations.sum_finalPriceEUR.request
											}
										}
									}
								}
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			stats.histogram_lots_awardDecisionDate = Queries.Aggregations.histogram_lots_awardDecisionDate_reverseNested.parse(answer_winning, parseoptions);
			stats.sum_finalPriceEUR = Queries.Aggregations.sum_finalPriceEUR.parse(answer_winning.tender_denested, parseoptions);
			stats.top_terms_authorities = Queries.Aggregations.top_terms_authorities.parse(answer_winning.tender_denested, parseoptions);
			stats.top_sum_finalPrice_authorities = Queries.Aggregations.top_sum_finalPrice_authorities.parse(answer_winning.tender_denested, parseoptions);
			stats.terms_authority_nuts = Queries.Aggregations.terms_authority_nuts.parse(answer_winning.tender_denested, parseoptions);
	...
	searchTender(options, country_id, cb) {
		...
		this.store.Tender.search(body, (options.size || 10), (options.from || 0), (err, result) => {
			...
			result.hits.hits = result.hits.hits.map(doc => {
	...
	searchBuyer(options, country_id, cb) {
		...
		this.store.Buyer.search(body, options.size || 10, options.from || 0, (err, result) => {
			...
			result.hits.hits = result.hits.hits.map(hit => hit._source);
	...
	fillAggregationAuthorityBodies(stats, cb) {
		...
		this.store.Buyer.getByIds(ids, (err, result) => {
			...
			result.hits.hits.forEach(hit => {
				results[hit._source.body.groupId] = hit._source.body;
			});

			if (stats.top_sum_finalPrice_authorities) {
				stats.top_sum_finalPrice_authorities.top10.forEach(entry => {
					entry.body = results[entry.groupId] || {};
		...
		this.store.Supplier.getByIds(ids, (err, result) => {
				...
				if (stats.top_sum_finalPrice_companies) {
					stats.top_sum_finalPrice_companies.top10.forEach(entry => {
						entry.body = results[entry.groupId] || {};
						if (!entry.body.groupId) {
							console.log('alarm1', entry);
						}
						entry.groupId = undefined;
					});
				}
		...
		let searchSimilarCompanyName = (name, ignoreIds) => {
			...
			this.store.Supplier.search(body, 100, 0, (err, results) => {
				...
				let similar = results.hits.hits.map(hit => {
					return {value: hit._source.sources.length, body: hit._source.body, country: hit._source.body.address.country};
				});
