...
const CPVs = require('./cpvs');
const Utils = require('./utils');
const Queries = require('./queries');

const Api = function (config) {
	let me = this;
	let cpvs = new CPVs(config);
	...
	this.getIndicatorStats = (options, country_id, cb) => {
		...
		store.Tender.search(b, 0, 0, (err, result) => {
			...
			let stats = aggs.parse(result.aggregations, cpvs, result.hits.total);
			...
			aggs = Queries.buildAggregations(['histogram_lots_awardDecisionDate', 'terms_main_cpvs']);
			b.aggregations = aggs.request;
			...
			store.Tender.search(b, 0, 0, (err, full_result) => {
				...
				let data = aggs.parse(full_result.aggregations, cpvs, full_result.hits.total);
				...
				let terms_pc_main_cpvs = {};
				let stats_terms_main_cpvs = stats['terms_main_cpvs'];
				let data_terms_main_cpvs = data['terms_main_cpvs'];
				Object.keys(stats_terms_main_cpvs).forEach(key => {
					...
					terms_pc_main_cpvs[key] = cpc;
				});
				stats.terms_pc_main_cpvs = terms_pc_main_cpvs;
	...
	this.getCPVStats = (options, country_id, cb) => {
		...
		let cpvInfo = cpvs.parseCPVs(id);
		if (!cpvInfo.cpv) {
			return cb(404);
		}

		let aggs = Queries.buildAggregations(['top_authorities', 'top_companies', 'sums_finalPrice', 'terms_main_cpvs_full', 'histogram_lots_awardDecisionDate', 'count_lots_bids']);
		...
		store.Tender.search(b, 0, 0, (err, result) => {
			...
			let stats = aggs.parse(result.aggregations, cpvs, result.hits.total);
			let parent_cpv = null;
			if (cpvInfo.parent_cpv) {
				parent_cpv = {id: id.slice(0, 2), name: cpvs.getCPVName(cpvInfo.parent_cpv.id, lang)};
			}
			cb(null, {sector: {id: id, name: cpvs.getCPVName(id, lang)}, parent: parent_cpv, stats: stats});
		});
	...
	this.getCompanyStats = (options, country_id, cb) => {
		...
		let aggs = Queries.buildAggregations(['terms_main_cpvs']);
		...
		store.Tender.search(b, 0, 0, (err, result) => {
			...
			let stats = aggs.parse(result.aggregations, cpvs);

			let answer_winning = result.aggregations.lotsbids.lotsbids_nested_filter.lotsbids_nested.lotsbids_nested_filter;

			stats.histogram_lots_awardDecisionDate = Queries.Aggregations.histogram_lots_awardDecisionDate_reverseNested.parse(answer_winning, cpvs);
			stats.sums_finalPrice = Queries.Aggregations.sums_finalPrice.parse(answer_winning.tender_denested, cpvs);
			stats.top_authorities = Queries.Aggregations.top_authorities.parse(answer_winning.tender_denested, cpvs);
	...
	this.getAuthorityStats = (options, country_id, cb) => {
		...
		let aggs = Queries.buildAggregations(['terms_main_cpvs', 'sums_finalPrice', 'histogram_lots_awardDecisionDate', 'count_lots_bids', 'top_winning_companies']);
		b.aggregations = aggs.request;
		store.Tender.search(b, 0, 0, (err, result) => {
			if (err) {
				console.log(err);
	...
	this.getCPVMain = (country_id, withStats, cb) => {
		let aggs = Queries.buildAggregations(['terms_main_cpvs']);
		let b = {
			'aggregations': aggs.request
		};
		let stats_aggs = null;
		if (withStats) {
			stats_aggs = Queries.buildAggregations(['top_authorities', 'top_companies', 'sums_finalPrice', 'terms_main_cpvs', 'histogram_lots_awardDecisionDate', 'count_lots_bids']);
			b.aggregations['terms_main_cpvs'].aggregations.cpvs_filter.aggregations.maincpvs.aggregations = {
				'tender_denested': {
					'reverse_nested': {},
		...
		store.Tender.search(b, 0, 0, (err, data) => {
			...
			let result = data.aggregations['terms_main_cpvs'].cpvs_filter.maincpvs.buckets.map(bucket => {
				let sector = {
					id: bucket.key,
				...
				return {
					sector: sector,
					stats: stats_aggs.parse(bucket.tender_denested, cpvs, bucket.tender_denested.doc_count)
				};
			});
			cb(null, result);
		});
	...
	this.getViz = (ids, country_id, cb) => {
		...
		async.forEachSeries(viz, (v, next) => {
			...
			if (v === 'sectors_count') {
				...
				me.getCPVMain(country_id, true, (err, data) => {
					if (err) return cb(err);
					result[v] = data;
					next();
				});
	...
	this.getCPVMainUsage = (country_id, cb) => {
		me.getCPVMain(country_id, false, cb);
	};

	this.getTenderPerYearStats = (country_id, cb) => {
		let aggs = Queries.buildAggregations(['histogram_lots_awardDecisionDate']);
		let b = {
			'aggregations': aggs.request
		};
		if (country_id) {
