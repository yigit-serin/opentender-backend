...
class Api {
	...
	getIndicatorStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			this.store.Tender.search(b, 0, 0, (err, full_result) => {
				...
				stats.histogram_indicators = data.histogram_indicators_para || data.histogram_indicator_para;
				stats.terms_main_cpv_divisions_score = data.terms_main_cpv_divisions_indicator_para || data.terms_main_cpv_divisions_score_para;

				stats.terms_indicators_score = data.terms_indicators_score_para;
				stats.terms_score = data.avg_score_para;
	...
	getCompany(id, country_id, cb) {
		...
		this.store.Supplier.get(id, (err, result) => {
			...
			result.hits.hits[0]._source.sources = undefined;
	...
	test(s, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			cb && cb();
			// result.hits.hits.forEach(hit => {
			// 	console.log(JSON.stringify({id: hit._source.id, score: hit._source.scores, indi: hit._source.indicators, indi_asfields: hit._source._indicators}));
			// });
	...
	init(cb) {
		this.store.init(err => {
			// this.test('nativescript', () => {
			// 	this.test('nativescriptList', () => {
			//
			// 	});
			// });
			cb(err);
