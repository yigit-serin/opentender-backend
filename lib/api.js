...
class Api {
	...
	getIndicatorStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, result) => {
			...
			aggs = Queries.buildAggregations([
				...
				'terms_indicators_score', 'avg_scores']);
			b.aggregations = aggs.request;
			...
			this.store.Tender.search(b, 0, 0, (err, full_result) => {
				...
				stats.histogram_lots_awardDecisionDate_scores = {};
				if (data.histogram_lots_awardDecisionDate_indicators) {
					Object.keys(data.histogram_lots_awardDecisionDate_indicators).forEach(key => {
						stats.histogram_lots_awardDecisionDate_scores[key] = data.histogram_lots_awardDecisionDate_indicators[key];
					});
				}
				if (data.histogram_lots_awardDecisionDate_scores) {
					Object.keys(data.histogram_lots_awardDecisionDate_scores).forEach(key => {
						stats.histogram_lots_awardDecisionDate_scores[key] = data.histogram_lots_awardDecisionDate_scores[key];
					});
				}
				...
				if (data.terms_main_cpv_divisions_scores) {
					Object.keys(data.terms_main_cpv_divisions_scores).forEach(key => {
						...
						if (part.scores.hasOwnProperty(indicator)) {
							let value = part.scores[indicator];
							if (value !== null) {
				...
				stats.terms_score = {};
				Object.keys(data.avg_scores).forEach(key => {
					if (indicator.indexOf(key) === 0) {
						stats.terms_score[key] = data.avg_scores[key];
					}
				});
	...
	getMarketAnalysisStats(options, country_id, cb) {
		...
		this.store.Tender.search(b, 0, 0, (err, data) => {
			...
			let sectors_stats = data.aggregations['terms_main_cpv_divisions'].cpvs_filter.cpvs.buckets.map(bucket => {
				...
				return {
					sector: sector,
					stats: cpv_stats_aggs.parse(bucket.tender_denested, {library: this.library, lang: options.lang})
				};
			});
	...
	getSectorStats(options, country_id, cb) {
		...
		let aggIds = [
			...
			'top_sum_finalPrice_authorities',
			'histogram_lots_awardDecisionDate_finalPriceEUR',
			'histogram_lots_awardDecisionDate_scores',
			'terms_procedure_type',
		...
		this.store.Tender.search(b, 0, 0, (err, data) => {
			...
			if (subaggregationId) {
				...
				let result = answer.cpvs_filter.cpvs.buckets.map(bucket => {
					...
					return {
						sector: sector,
						stats: stats_aggs.parse(bucket.tender_denested, {library: this.library, lang: options.lang})
					};
				});
	...
	getRegionStats(options, country_id, cb) {
		...
		let aggs = Queries.buildAggregations([
			...
			'terms_main_cpv_divisions',
			'histogram_lots_awardDecisionDate']);
		if (nutsInfo.nuts.level < 4) {
			let term = {};
			let field = 'buyers.address.nutscode.nuts' + nutsInfo.nuts.level;
				...
				'aggregations': {
					'buyers_filter': {
						...
						'aggregations': {
							'nuts': {
								'terms': {
									'field': 'buyers.address.nutscode.nuts' + (nutsInfo.nuts.level + 1),
									'size': 300000
								}
							}
						}
			...
			if (result.aggregations.terms_subregions) {
				...
				buckets.forEach(bucket => {
					...
					if (code !== nutsInfo.nuts.id) {
						stats.terms_subregions_nuts[code] = bucket.doc_count;
						children.push({
							id: code,
	...
	getHomeStats(country_id, cb) {
		this.getAggregations(country_id, ['histogram_lots_awardDecisionDate'], 'en', cb);
	}
	...
	autocomplete(entity, field, search, country_id, cb) {
		...
		let hasRaw = () => {
			//TODO: reimport fundings with .raw
			return ((field.indexOf('fundings.programme')) !== 0) && ((field.indexOf('fundings.source')) !== 0);
		};
		...
		if (country_id) {
			body.query.bool.filter.push({term: country_term});
		}
		if (query_field) {
