...
const Api = function (config) {
	...
	this.getAuthorityStats = (options, country_id, cb) => {
		...
		b.query = Queries.addCountryFilter(b.query, country_id);
		let aggs = Queries.buildAggregations(['terms_main_cpv_divisions', 'sums_finalPrice', 'histogram_lots_awardDecisionDate', 'count_lots_bids', 'top_winning_companies']);
		b.aggregations = aggs.request;
		store.Tender.search(b, 0, 0, (err, result) => {
			if (err) {
				console.log(err);
	...
	this.getMarketAnalysisStats = (options, country_id, cb) => {
		...
		aggs.request['terms_main_cpv_divisions'].aggregations.cpvs_filter.aggregations.maincpvs.aggregations = {
			'tender_denested': {
				'reverse_nested': {},
		...
		store.Tender.aggregations(country_id, aggs.request, (err, data) => {
			...
			let result = data.aggregations['terms_main_cpv_divisions'].cpvs_filter.maincpvs.buckets.map(bucket => {
				let sector = {
					id: bucket.key,
